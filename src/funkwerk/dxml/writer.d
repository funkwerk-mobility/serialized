...
// Vendored into `serialized` to do some performance improvements:
// for instance, tag validation is unnecessary for a structured serialization lib.
...
public:
    ...
    {
        import funkwerk.dxml.internal : TestAttrOR;
    ...
    static if(compileInTests) unittest
    {
        import std.array : appender;
        import std.exception : assertThrown;
        import funkwerk.dxml.internal : testRangeFuncs;
...
void checkName(R)(R range)
{
    ...
    import funkwerk.dxml.internal : isNameStartChar, isNameChar;
    ...
    loop: while(!text.empty)
    {
        switch(text.front)
        {
            ...
            default:
            {
                ...
                import funkwerk.dxml.internal : isXMLChar;
                immutable c = text.front;
                if(isASCII(c))
                {
                    if(!isXMLChar(c))
